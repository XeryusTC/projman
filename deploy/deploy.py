...
def _update_virtualenv(env):
    ...
    run('{venv}/bin/pip install -r {source}/requirements/prod.txt'.format(
        venv=env.virtualenv_dir, source=env.source_dir))

def _update_static_files(env):
    with _enable_env_vars(env):
        run('cd {source} && ../virtualenv/bin/python3 manage.py \
                collectstatic --noinput', source=env.source_dir)
